...
export function SignInForm({ locale }: { locale: string }) {
  ...
  return (
    ...
    >
      <SignInFormSteps locale={locale} />
    </SignIn.Root>
  );
}

function SignInFormSteps({ locale }: { locale: string }) {
  const t = useTranslations("auth");
  const c = useTranslations("common");
  const isCompletingSession = useSignInCompleting();
  ...
  return (
    <>
      ...
      <Clerk.Loading scope="global">
        {(isGlobalLoading) => {
          const showCompletingLoader =
            isGlobalLoading || isFinishingSignIn || isCompletingSession;

          return showCompletingLoader ? (
            <AuthLoadingIndicator message={t("signIn.completingSignIn")} />
          ) : (
            <>
              ...
              <SignIn.Step name="verifications">
                <SignIn.Strategy name="email_code">
                  <SignInSecondFactorPrepare />
                  <SignInOtpFinishLatch onFinish={handleOtpFinish} />
                  <div className="space-y-4">
                      ...
                      {(isLoading) => (
                        <SignIn.Action submit asChild>
                          <Button
                            ...
                            disabled={isLoading}
                          >
                            <span className="inline-flex min-w-[5.5rem] items-center justify-center">
                              {isLoading ? (
                                <Loader2 className="size-4 animate-spin" />
                              ) : (
