...
export function SidebarQaTesting({
  ...
  const { isQaTester, isPlatformAdmin, issueTrackerEnabled } = useWorkspaceContext();
  const collapsedFromStore = useSidebarStore((s) => s.collapsed);
  const qaOpen = useSidebarStore((s) => s.sectionsOpen.qa);
  const toggleSection = useSidebarStore((s) => s.toggleSection);
  const collapsed = collapsedOverride ?? collapsedFromStore;
  const { inDrawer } = useSidebarLayout();

  if (!issueTrackerEnabled || !isQaTester || isPlatformAdmin) {
    return null;
  }
      ...
      >
        {!collapsed ? (
          <SidebarSectionLabel
            ...
            toggleLabel={qaOpen ? t("qaTesting.collapse") : t("qaTesting.expand")}
            className="pt-1"
          >
            {t("qaTesting.title")}
          </SidebarSectionLabel>
