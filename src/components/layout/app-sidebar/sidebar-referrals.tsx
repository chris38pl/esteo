...
import { dashboardReferralsHref } from "@/lib/dashboard-routes";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { cn } from "@/lib/utils";

export function SidebarReferrals({
  collapsedOverride,
}: {
  ...
  const { partnerProgramVisible, activeWorkspace, locale } = useWorkspaceContext();
  ...
  if (!partnerProgramVisible || !activeWorkspace?.slug) {
    return null;
  }

  const href = dashboardReferralsHref(locale, activeWorkspace.slug);
  const active = pathname === href || pathname.startsWith(`${href}/`);
  const label = t("referrals");
  ...
  const row = collapsed ? (
    ...
    >
      <Handshake className="size-3.5 shrink-0 opacity-80" strokeWidth={1.75} />
    </Link>
  ) : (
    <Link
      href={href}
    ...
    >
      <Handshake className="size-3.5 shrink-0 opacity-80" strokeWidth={1.75} aria-hidden />
