...
import { toggleTipPin } from "@/features/tips/lib/tips-storage";
...
export function TipsListPanel({ locale, workspaceSlug }: TipsListPanelProps) {
  ...
  const { pinnedIds, refreshTipsStorage } = useTipsStorageState(currentUserId, workspaceSlug);

  function handlePinToggle(tipId: (typeof TIPS_CATALOG)[number]["id"]) {
    const result = toggleTipPin(currentUserId, workspaceSlug, tipId);
