...
interface EstimatesListDateRangePickerProps {
  locale: Locale;
  value: EstimateListDateRange;
  onChange: (range: EstimateListDateRange) => void;
}

const DATE_FIELDS: EstimateListDateField[] = ["updated", "created", "requestCreated"];
...
function toDateRange(value: EstimateListDateRange): DateRange | undefined {
...
function fromDateRange(field: EstimateListDateField, range: DateRange | undefined): EstimateListDateRange {
  return {
    field,
...
export function EstimatesListDateRangePicker({
  locale,
  value,
  onChange,
}: EstimatesListDateRangePickerProps) {
  const t = useTranslations("estimates");
  const isMobile = useEstimateMobileLayout();
  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState<EstimateListDateRange>(value);
  ...
  const clearDraft = () => {
    setDraft(EMPTY_ESTIMATE_LIST_DATE_RANGE);
    onChange(EMPTY_ESTIMATE_LIST_DATE_RANGE);
    setOpen(false);
  };
