...
import {
  ...
  ESTIMATE_LIST_FILTER_FIELDS,
  ESTIMATE_LIST_STATUS_VALUES,
  countMatchingEstimates,
  createListFilterCondition,
  isStatusListFilterField,
  isTextListFilterField,
  operatorsForListField,
  type EstimateListDateRange,
  type EstimateListFilterCondition,
  type EstimateListFilterField,
  type EstimateListFilterLogic,
  type EstimateListFilterState,
...
interface EstimatesListFilterSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  estimates: EstimateListPageItem[];
  searchQuery: string;
  appliedDateRange: EstimateListDateRange;
  appliedFilter: EstimateListFilterState;
  onApply: (filter: EstimateListFilterState) => void;
}
...
function FilterFieldIcon({ field }: { field: EstimateListFilterField }) {
  ...
  const Icon = isTextListFilterField(field) ? Type : Hash;
  return (
    <span className="estimate-mobile-position-icon flex size-9 shrink-0 items-center justify-center rounded-lg">
...
export function EstimatesListFilterSheet({
  open,
  onOpenChange,
  estimates,
  searchQuery,
  ...
  appliedFilter,
  onApply,
}: EstimatesListFilterSheetProps) {
  const t = useTranslations("estimates");
  const isMobile = useEstimateMobileLayout();
  const [draft, setDraft] = useState<EstimateListFilterState>(appliedFilter);
  ...
  const addableFields = ESTIMATE_LIST_FILTER_FIELDS.filter((field) => !usedFields.has(field));

  const previewCount = countMatchingEstimates(estimates, {
    searchQuery,
    filter: draft,
    dateRange: appliedDateRange,
  });

  const updateCondition = (id: string, patch: Partial<EstimateListFilterCondition>) => {
    setDraft((prev) => ({
      ...prev,
  ...
  const addCondition = (field: EstimateListFilterField) => {
    setDraft((prev) => ({
      ...prev,
      conditions: [...prev.conditions, createListFilterCondition(field)],
    }));
    setFieldPickerOpen(false);
  };

  const clearDraft = () => {
    setDraft(EMPTY_ESTIMATE_LIST_FILTER);
    onApply(EMPTY_ESTIMATE_LIST_FILTER);
    onOpenChange(false);
  };
    ...
    <div className="flex-1 space-y-5 overflow-y-auto px-4 py-4 sm:px-5">
          ...
          {draft.conditions.map((condition) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(event) =>
                    updateCondition(condition.id, {
                      operator: event.target.value as EstimateListFilterCondition["operator"],
                    })
                  }
                  className={selectClassName}
                  aria-label={t("list.filter.operatorAria")}
                >
                  {operatorsForListField(condition.field).map((operator) => (
                ...
                {isStatusListFilterField(condition.field) ? (
                  <select
                  ...
                  >
                    ...
                    {ESTIMATE_LIST_STATUS_VALUES.map((status) => (
      ...
      <section className="space-y-2.5">
        ...
        <div className="grid grid-cols-2 gap-2">
          {(["and", "or"] as EstimateListFilterLogic[]).map((logic) => {
