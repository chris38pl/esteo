...
import { Clock, Coins, LayoutGrid, Loader2, MoreHorizontal, Pencil, Star, StarOff, Trash2 } from "lucide-react";
...
import { PriceListMetadataDialog } from "@/features/price-lists/components/price-list-metadata-dialog";
...
interface PriceListDetailHeaderProps {
  name: string;
  currency: string;
  isDefault: boolean;
  isNew: boolean;
  ...
  itemCount: number;
  onMetadataSave: (payload: { name: string; currency: string }) => void | Promise<void>;
}

const controlButtonClass = "h-10 rounded-md px-4";
...
const kpiCurrencyValueClass = "min-h-8 text-lg font-semibold tabular-nums leading-8";
...
export function PriceListDetailHeader({
  ...
  const editButton = (
    <Button
      type="button"
      variant="outline"
      className={cn(controlButtonClass, "gap-2")}
      disabled={readOnly}
        ...
        <DropdownMenuContent align="end">
          {isDefault ? (
            ...
            <DropdownMenuItem
              ...
              onClick={async () => {
                const result = await setDefaultPriceListAction(
                  { workspaceId, workspaceSlug, priceListId },
                  locale,
                );
                if (!result.success) appToast.error(result.error);
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 min-h-5 text-sm leading-relaxed text-muted-foreground">
              {currency.trim() || tWorkspace("noCurrency")}
            </p>
          </div>
        ...
        <div className="mt-6 mb-6 overflow-hidden rounded-lg border border-border/70">
          <div className="grid divide-y divide-border/60 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            ...
            <div className={kpiCellClass}>
              ...
              <div>
                <p className="text-xs text-muted-foreground">{tWorkspace("kpiCurrency")}</p>
                <p className={kpiCurrencyValueClass}>{currency.trim() || "—"}</p>
              </div>
            </div>
            <div className={kpiCellClass}>
              ...
              <div>
                ...
                <p className={kpiCurrencyValueClass}>{formattedUpdatedAt ?? "—"}</p>
              </div>
            </div>
          </div>
        </div>
...
export function PriceListDefaultNotice() {
  const tWorkspace = useTranslations("workspaces.configuration.priceLists.workspace");
