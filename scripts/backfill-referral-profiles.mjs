...
if (useStaging) {
  ...
  console.log("Backfilling referral profiles on Neon staging branch…");
} else {
  if (!databaseUrl) {
  ...
  console.log("Backfilling referral profiles on default DATABASE_URL (development)…");
}
...
const result = spawnSync("npx", ["tsx", "scripts/backfill-referral-profiles.ts"], {
  stdio: "inherit",
  env,
  shell: true,
});
