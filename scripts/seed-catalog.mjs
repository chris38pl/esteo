...
if (useProduction) {
  ...
  console.log("Seeding platform catalog on Neon staging branch…");
} else {
  if (!databaseUrl) {
  ...
  console.log("Seeding platform catalog on default DATABASE_URL (development)…");
}
...
const result = spawnSync("npx", ["tsx", "prisma/seed-catalog.ts"], {
  stdio: "inherit",
  env,
  shell: true,
});
